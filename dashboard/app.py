...
def trade_entry_panel():
    return html.Div(
        ...
        children=[
            input_box("Spread ID",      "p-spread-id", "1",     width="60px"),
...
def _chart_block(b64_src, title):
    """Render a pre-generated chart image, or a placeholder if missing."""
    if b64_src:
        return html.Div(
    ...
    return html.Div(
        style={
            ...
            "color": "#555", "fontFamily": "Courier", "fontSize": "13px",
        },
        children=f"⚠ {title} not found — run main.py to generate charts before starting.",
    )
...
def refresh_spreads(_):
    ...
    try:
        ...
        if call_df.empty:
            call_tables = [html.P("No call spreads match filters — greeks may still be loading.",
                                  style={"color": "#aaa"})]
        else:
    ...
    try:
        ...
        if put_df.empty:
            put_tables = [html.P("No put spreads match filters — greeks may still be loading.",
                                 style={"color": "#aaa"})]
        else:
...
@app.callback(
    ...
    State("p-spread-id", "value"),
    State("p-exchange",  "value"),
    State("p-symbol",    "value"),
    State("p-token",     "value"),
    prevent_initial_call=True,
)
def log_trade_cb(buy_clicks, sell_clicks, spread_id, exchange, symbol, token):
    if not exchange or not symbol or not token:
        return "⚠ Fill all fields"
    if not spread_id:
        return "⚠ Enter a Spread ID"
    try:
        spread_id = int(spread_id)
    except ValueError:
        return "⚠ Spread ID must be a number"
    try:
        ...
        log_trade(exchange, symbol, token, side, ltp, spread_id=spread_id)
        return f"✓ {side} {symbol} @ {ltp}  [spread {spread_id}]"
    except Exception as e:
        return f"✗ {e}"
...
def refresh_portfolio(_):
    ...
    pnl_block = html.Div([
        html.Span("Total Portfolio PnL  ",
                  style={"color": "#aaa", "fontFamily": "Courier", "fontSize": "14px"}),
        html.Span(f"{pnl_sign}₹{total_pnl:,.2f}",
