...
class IndiaCreditSpreads:
    ...
    def __init__(self, broker, connection):
        ...
        self.raw_calls = None
        self._base_calls = None   # fresh filtering each cycle
    ...
    def start_spread_feed(self):
        token_list = [{"exchangeType": 2, "tokens": self.spread_tokens}]
        def on_data(wsapp, message):
            self.spread_latest[message['token']] = message
    ...
    def apply_greeks_filters(self, pmcc_greeks_cache):
        """
        Called from app.py inside the PMCC greeks refresh loop.
        Reuses pmcc.greeks_cache — no extra API calls.
        Applies delta and bid-ask spread filters.
        """
        ...
        # attach bid/ask from live ticks
        rows = []
